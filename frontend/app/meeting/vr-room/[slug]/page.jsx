...
import { ConferenceTable } from "../models/ConferenceTable";
import { SittingSoham } from "../models/Sitting-soham";
import { SohamR } from "../models/Soham-r-model";
import { SohamPModel } from "../models/Soham-P-model";
...
const VRCanvas = () => {
  const { setIsHeadFollow, setMyAnimation } = useVRStore();
  ...
  useEffect(() => {
    setIsHeadFollow(headFollow);
    setMyAnimation(animation);
  }, [headFollow, animation]);
      ...
      >
        <XR>
          ...
          <group position={[0, 1, 0]} rotateOnWorldAxis={[0, Math.PI / 2, 0]}>
            <Environment
              ground={{ scale: 30, radius: 130 }}
              files="/environments/shanghai_bund_2k.hdr"
              background
              blur={0}
            />
          </group>
